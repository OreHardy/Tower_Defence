...
void dessineAttaque(SDL_Surface *surface, Tunite *attaquant, Tunite *cible, Uint32 colorChoisie){

    //colorChoisieHorde=0xFF0000;
    //colorChoisieTour=0x008000;
    ...
    */
    line(surface, x1, y1, x2, y2, colorChoisie);
    //SDL_Delay(250);
}

void dessineSpawn(SDL_Surface *surface, Tunite *horde, int rayon, Uint32 colorChoisie){
    int centerX = (horde->posX+1)*40-20;
    int centerY = (horde->posY+1)*40-20;
    int rayon2 = (rayon+1)*40-20;

    circle(surface, centerX, centerY, rayon2, colorChoisie);
    //SDL_Delay(500);
}

void dessineCase(SDL_Surface *surface, int xtour, int ytour, Uint32 colorChoisie){
    int w = (1+1)*40-40;
    int h = (1+1)*40-40;
    xtour = (xtour)*40;
    ytour = (ytour)*40;

    box(surface,xtour, ytour, w, h, colorChoisie);
}
